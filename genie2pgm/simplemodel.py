...
class SimpleDiscreteModel(BasicOperation):
    ...
    def getInterface(self) -> tuple[list, str]:  # -> tuple[list, str | Any | None]:
        """
        @Description: 获取类的输入、输出节点
        """
        inputNodes = list()
        # only one outputNode
        outputNodes = None
        for node in self.nodes:
            properties = node.find("property")
            if properties is not None and properties.attrib.get("id") == "interface":
                if properties.text == "output":
                    outputNodes = node.attrib.get("id")
                elif properties.text == "input":
                    inputNodes.append(node.attrib.get("id"))
        ...
        for node in self.nodes:
            ...
            if node.findall("parents"):
                ...
                if not value_dict:
                    ...
                    """
                    The number of lists is the number of node states, and the number of elements in the list is the number of permutations of parent node states
                    """
