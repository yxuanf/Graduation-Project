...
class NoisyMax(SimpleDiscreteModel):
        ...
        for i in range(node_state):
            ...
            for j in range(p_state**parent_num):
                ...
                for k in range(parent_num):
                    copy[k][int(self.decimal2base(j, p_state, parent_num)[k])][
                        i
                    ] = param[k][int(self.decimal2base(j, p_state, parent_num)[k])][
                        i
                    ] / sum(
                        param[k][int(self.decimal2base(j, p_state, parent_num)[k])][i:]
                    )
        return copy
    ...
    def noisymax2cpd(self, node):
        ...
        # prob tabel -> matrix
        param = self.list2matrix(
            [param[i : i + state_num] for i in range(0, len(param), state_num)]
        )
        # leaked parameters
        leak = param[-1]
        # delete leak from prob tabel
        param = np.delete(param, -1, axis=0).reshape(parent_num, p_state_num, state_num)
        # Noisymax probaility process
        for i in range(len(leak)):
            leak[i] = leak[i] / sum(leak[i:])
        param = np.nan_to_num(
            self.prob_tabel_process(param, parent_num, p_state_num, state_num)
        )
        tabel = self.calculate_cpd(param, leak, state_num, p_state_num, parent_num)
        return tabel.tolist()
